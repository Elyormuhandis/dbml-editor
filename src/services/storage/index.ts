...
export const saveEditorCollapsed = (collapsed: boolean): void => {
  try {
    localStorage.setItem(STORAGE_KEYS.EDITOR_COLLAPSED, JSON.stringify(collapsed));
  } catch (error) {
    console.error('Failed to save editor collapsed state to localStorage:', error);
  }
};
...
export const loadEditorCollapsed = (): boolean | null => {
  try {
    ...
    console.error('Failed to load editor collapsed state from localStorage:', error);
  }
  return null;
};
